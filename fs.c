...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDINDIRECT)
  {
	  // Load first level from double indirect block, allocating if necessary.
	 ...
	 // cprintf( "entry: %d, bn: %d \n", entry, bn);
	  ...
	  // load second level from double indirect block
	  bp_level2 = bread(ip->dev, addr);
	  a = (uint*)bp_level2->data;
